...
import 'package:pizzer_mobile/blocs/worker_orders/worker_orders_bloc.dart';
...
class WorkerOrdersPage extends StatelessWidget {
  String? token;
  WorkerOrdersPage({super.key, this.token});

  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<WorkerOrdersBloc>(
          create: (BuildContext context) =>
              WorkerOrdersBloc(OrderRepository(), UserInfoRepository()),
        ),
      ],
      child: blocBody(),
    );
  ...
  Widget blocBody() {
    return BlocProvider(
      create: (context) =>
          WorkerOrdersBloc(OrderRepository(), UserInfoRepository())
            ..add(LoadWorkerOrderEvent(token)),
      child: BlocBuilder<WorkerOrdersBloc, WorkerOrdersState>(
        builder: (context, state) {
          if (state is WorkerOrderLoadingState) {
            return const Center(
          ...
          if (state is WorkerOrdersLoadedState) {
            return workerOrdersLoaded(context, state);
          }
          if (state is ChosenOrderLoadedState) {
            return chosenOrderLoaded(context, state);
          }
          if (state is WorkerNoOrdersState) {
            return workerNoOrder(context, state);
          }
          if (state is WorkerOrdersErrorState) {
            return const Center(
              child: Text("Error"),
            );
          }
  ...
  Widget workerOrdersLoaded(context, state) {
      ...
      itemBuilder: (_, index) {
        return Padding(
          ...
          child: GestureDetector(
            onTap: () => BlocProvider.of<WorkerOrdersBloc>(context)
                        ...
                        Expanded(
                            child: Padding(
                                ...
                                child: GestureDetector(
                                    ...
                                    onTap: () => {
                                          BlocProvider.of<WorkerOrdersBloc>(
                                                  context)
  ...
  Widget chosenOrderLoaded(context, state) {
    ...
    return WillPopScope(
        onWillPop: () {
          BlocProvider.of<WorkerOrdersBloc>(context)
              .add(LoadWorkerOrderEvent(token));
  ...
  Widget workerNoOrder(context, state) {
